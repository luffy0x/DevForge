...
Execute one queued task and open a draft PR:
...
`contribute` claims the queued task itself; do not run `devforge claim` first.

### Contribute through a fork

Scan issues from the upstream repository, then ensure your fork is synced to its target base branch. Pass the fork as both the workspace URL and publication repository:

```bash
python -m devforge contribute \
  --task opensandbox-group/OpenSandbox#123 \
  --repository-url https://github.com/your-account/OpenSandbox.git \
  --publish-repository your-account/OpenSandbox \
  --branch devforge/issue-123
```

DevForge creates the branch and commits in the fork, then creates a Draft PR from `your-account:devforge/issue-123` to `opensandbox-group/OpenSandbox`.
